...
export function Checkout() {
  ...
  const handleCompleteOrder = (e: React.FormEvent) => {
    e.preventDefault();
    if (user) {
      addOrder({ userId: user.id, items: [...cart], total });
    }
    setStep(3);
    clearCart();
    setTimeout(() => {
              ...
              <div className="space-y-6">
                <div>
                  <div className="flex justify-between items-center mb-4">
                    ...
                    <button type="button" onClick={() => setStep(1)} className="text-sm font-medium text-gray-600 hover:text-gray-900">Edit Shipping</button>
                  </div>
                  <div className="space-y-4 mb-6">
                    <label className="flex items-center p-4 border border-gray-200 rounded-sm cursor-pointer hover:bg-gray-50">
                      <input type="radio" name="payment" value="cod" checked={paymentMethod === 'cod'} onChange={() => setPaymentMethod('cod')} className="form-radio text-gray-900 focus:ring-gray-900 h-4 w-4" />
                      <span className="ml-3 text-sm font-medium text-gray-900">Cash on Delivery</span>
                    </label>
                    <label className="flex items-center p-4 border border-gray-200 rounded-sm cursor-pointer hover:bg-gray-50">
                      <input type="radio" name="payment" value="easypaisa" checked={paymentMethod === 'easypaisa'} onChange={() => setPaymentMethod('easypaisa')} className="form-radio text-gray-900 focus:ring-gray-900 h-4 w-4" />
                      <span className="ml-3 text-sm font-medium text-gray-900">Easypaisa</span>
                    </label>
                    <label className="flex items-center p-4 border border-gray-200 rounded-sm cursor-pointer hover:bg-gray-50">
                      <input type="radio" name="payment" value="card" checked={paymentMethod === 'card'} onChange={() => setPaymentMethod('card')} className="form-radio text-gray-900 focus:ring-gray-900 h-4 w-4" />
                  ...
                  {paymentMethod === 'cod' && (
                    <div className="p-4 bg-gray-50 text-sm text-gray-600 rounded-sm border border-gray-200">
                  ...
                  {paymentMethod === 'easypaisa' && (
                    <div className="p-4 bg-green-50 border border-green-200 rounded-sm">
                      ...
                      <div>
                        ...
                        <input type="text" required className="w-full px-3 py-2 border border-green-300 rounded-sm focus:outline-none focus:ring-1 focus:ring-green-500 bg-white" placeholder="e.g. 1234567890" />
                      </div>
                    </div>
                  )}
                </div>
