...
export function Cart() {
  ...
  return (
    ...
    >
      <h1 className="text-3xl font-bold tracking-tight text-gray-900 dark:text-white mb-12">Shopping Cart</h1>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-8">
        <div className="lg:col-span-8">
          <ul className="divide-y divide-gray-200 border-t border-gray-200 dark:border-gray-700">
            {cart.map((item) => (
              <li key={item.product.id} className="py-6 flex">
                <div className="w-24 h-24 flex-shrink-0 bg-gray-100 dark:bg-gray-800 rounded-md overflow-hidden">
                ...
                <div className="ml-4 flex flex-1 flex-col justify-between">
                  <div>
                    <div className="flex justify-between text-base font-medium text-gray-900 dark:text-white">
                      ...
                      <p className="ml-4">Rs. {(item.product.price * item.quantity).toFixed(2)}</p>
                    </div>
                  ...
                  <div className="flex items-center justify-between text-sm">
                    <div className="flex items-center border border-gray-200 dark:border-gray-700 rounded">
                      <button onClick={() => updateQuantity(item.product.id, Math.max(1, item.quantity - 1))} className="p-1.5 text-gray-400 hover:text-gray-900 dark:text-white">
                        <Minus className="w-3 h-3" />
                      </button>
                      <span className="w-8 text-center text-sm font-medium text-gray-900 dark:text-white">{item.quantity}</span>
                      <button onClick={() => updateQuantity(item.product.id, item.quantity + 1)} className="p-1.5 text-gray-400 hover:text-gray-900 dark:text-white">
        ...
        <div className="lg:col-span-4">
          <div className="bg-gray-50 dark:bg-gray-800 rounded-lg px-4 py-6 sm:p-6 lg:p-8">
            ...
            <div className="flow-root">
              <dl className="-my-4 text-sm divide-y divide-gray-200">
                ...
                <div className="py-4 flex items-center justify-between">
                  ...
                  <dd className="text-base font-medium text-gray-900 dark:text-white">Rs. {total.toFixed(2)}</dd>
                </div>
              </dl>
            </div>
            <div className="mt-8">
